...
- **`centerline.py`** — конвертує будь-який контурний шрифт TTF/OTF в однолінійний Hershey SVG-шрифт. Саме так отримати кирилицю: взяти будь-який шрифт з підтримкою української (Caveat, Kobzar KS тощо) і конвертувати.
- **`text_to_gcode.py`** — переводить вміст `.txt`-файлу безпосередньо в G-code посторінково: перенос слів, пагінація, портретна/ландшафтна орієнтація, точні відступи — все автоматично.
...
## editor/editor.py — веб-редактор з попереднім переглядом і друком

Flask-застосунок із браузерним інтерфейсом: зліва — поле введення тексту, справа — SVG-превʼю сторінки в реальному часі. Замінює ланцюжок `text_to_gcode.py → plot.py` для інтерактивної роботи.
...
```bash
python editor/editor.py --font fonts/myfont_centerline/myfont_centerline.svg
```

Відкрий у браузері: `http://127.0.0.1:5000/`

Усі параметри розмітки аналогічні `text_to_gcode.py`:

```bash
python editor/editor.py \
    --font fonts/myfont_centerline/myfont_centerline.svg \
    --orientation landscape \
    --size 5 --line-height 7 --line-gap 2 \
    --margin-left 73 --margin-right 80 --margin-top 42 --margin-bottom 20 \
    --port 5000
...
| Елемент | Призначення |
|---------|-------------|
| Ліва панель | Введення тексту; текст зберігається в `localStorage` між сесіями |
| Права панель | SVG-превʼю поточної сторінки (оновлюється через 400 мс після зупинки введення) |
| **Start line** | Скільки рядків пропустити на першій сторінці (якщо папір уже частково надрукований) |
| **Сітка** | Перемикає показ сітки-лінійки та контуру текстової області в прев'ю (не впливає на друк) |
| **‹ / ›** / поле сторінки | Навігація між сторінками; також `Alt+←` / `Alt+→` |
| **Друкувати** | Генерує G-code через `vpype` і запускає `plot.py` для поточної сторінки |
| **Пауза / Продовжити** | Надсилає Enter у stdin `plot.py` (пауза/відновлення між ходами) |
| **Стоп** | Надсилає `SIGINT` процесу `plot.py`; перо піднімається і повертається додому |

Ширина лівої панелі регулюється перетягуванням розділювача.

---

## grbl_tune.py — перегляд і налаштування параметрів GRBL

Підключається до плотера, зчитує `$$` і виводить параметри, що впливають на швидкість друку. З прапором `--set` дозволяє змінити їх прямо в терміналі.

```bash
# Тільки переглянути поточні значення
python grbl_tune.py

# Переглянути і змінити
python grbl_tune.py --set
```
...
| Параметр | Опис |
|----------|------|
| `$110` / `$111` | Максимальна швидкість X / Y (мм/хв) |
| `$112` | Максимальна швидкість Z (мм/хв) |
| `$120` / `$121` | Прискорення X / Y (мм/с²) |
| `$122` | Прискорення Z (мм/с²) |

> Зміни зберігаються в EEPROM плотера і зберігаються після вимкнення.
