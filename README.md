...
# Бічні мітки (дата, автор тощо) — див. розділ "Бічні мітки" нижче
python text_to_gcode.py poem.txt --font fonts/myfont_centerline/myfont_centerline.svg \
    --label дата:-32:60 --label автор:+5:70
```
...
| `--label NAME:OFFSET:WIDTH` | — | Оголосити бічну мітку (повторюваний прапорець). Див. "Бічні мітки" нижче |
...
### Бічні мітки (дата, автор тощо)

Прапорець `--label ІМ'Я:ЗМІЩЕННЯ:ШИРИНА` (можна вказувати кілька разів) оголошує директиву `--ІМ'Я ...`, яку можна використовувати у вхідному текстовому файлі:

```
--дата 14.07.2026
--автор Іван Петренко

Цей рядок буде надруковано в основному тексті.
```

- Директива вилучається з основного тексту; її вміст друкується **збоку**, прив'язаний до наступного змістовного рядка.
- `ЗМІЩЕННЯ` (мм) — відступ від межі текстової області назовні, у поле сторінки: від'ємне значення рахується від `--margin-left`, додатне — від `--margin-right`.
- `ШИРИНА` (мм) — ширина бічного блоку; якщо текст мітки не влазить в один рядок, він переноситься на наступні рядки (baseline-слоти), не залежно від того, скільки рядків займає сам основний текст.
- Кілька міток підряд перед одним рядком — незалежні бічні колонки на тому самому якорі.
- Директива для імені, яке не було оголошене через `--label`, лишається звичайним текстом.
...
Усі параметри розмітки аналогічні `text_to_gcode.py`, включно з `--label` (див. "Бічні мітки" вище):
...
| **Сітка** | Перемикає показ сітки-лінійки, контуру текстової області та боксів бічних міток (з `--label`) в прев'ю (не впливає на друк) |
